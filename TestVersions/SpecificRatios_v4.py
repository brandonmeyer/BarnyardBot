...
class AnimalAI(gym.Env):
    ...
    def reset(self):
        ...
        #         if self.ratios[i] != 0:
        #             negReward += 2*abs(checkArr[i] - self.resourceCounter[i])
        #     checkArr = checkArr + self.ratios
        #     tempReward = 0;
        #     for i in range(self.ratios.shape[0]):
        #         if self.ratios[i] != 0:
        #             tempReward += 2*abs(checkArr[i] - self.resourceCounter[i])
        #     if(tempReward < negReward):
        #         negReward = tempReward
        #     print(self.totalReward, " - ", negReward)
        #     self.totalReward -= negReward
        print("rc:", self.resourceCounter)
        self.logResources.append(self.resourceCounter)
        self.startup = 1
        ...
        with open('animalai_returns.txt', 'w') as f:
            for step, value in zip(self.stepList[1:], self.rewardList[1:]):
                f.write("{}\t{}\n".format(step, value)) 
        if(self.startup != 0):
            with open('animalai_counts.txt', 'w') as f:
                for step, rc in zip(self.stepList[1:], self.logResources):
                    if len(rc) > 0:
                        f.write("{}\t{}\t{}\t{}\n".format(step, rc[0], rc[1], rc[2])) 
                    else:
                        f.write("{}\t{}\t{}\t{}\n".format(step, 0, 0, 0))
            ...
            for h in range(2,9):
                ...
                if obsJson[hotbItem] == "wool":
                    ...
                    if wool_color == "RED":
                        ...
                        reward += (wool_amount * self.redReward)
                        if(self.first[1]):
                    ...
                    if wool_color == "BLUE":
                        ...
                        reward += (wool_amount * self.blueReward)
                        if(self.first[2]):       
    ...
    def readRatios(self):
        ...
        ratioTot = np.sum(self.ratios)
        if self.ratios[0] == 0:
            self.milkReward = -1
        else:
            self.milkReward = self.ratios[0] / ratioTot
        if self.ratios[1] == 0:
            self.redReward =  -1
        else:
            self.redReward = self.ratios[1] / ratioTot
        if self.ratios[2] == 0:
            self.blueReward = -1
        else:
            self.blueReward = self.ratios[2] / ratioTot
